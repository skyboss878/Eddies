...
const CreateInvoice = () => {
  ...
  const getDefaultPrice = (type) => {
    if (type === "service" || type === "labor") {
      return settings.pricing?.laborRate || 140;
    }
    return 0;
  };
  ...
  const [invoiceData, setInvoiceData] = useState({
    ...
    items: [],
    notes: '',
    taxRate: settings.pricing?.taxRate * 100 || 8.75,
    discountAmount: 0,
    discountType: 'amount', // amount or percentage
    hazardousWasteFee: 0,
    shopSuppliesFee: settings.pricing?.shopSuppliesRate * 100 || 5
  });
  ...
  const [newItem, setNewItem] = useState({
    ...
    description: '',
    quantity: 1,
    unitPrice: getDefaultPrice("service"),
    taxable: true
  });
  ...
  const addItem = () => {
    ...
    const item = {
      ...newItem,
      id: Date.now(),
      total: newItem.type === "part" ? newItem.quantity * newItem.unitPrice * (1 + (settings.pricing?.partsMarkup || 0.35)) : newItem.quantity * newItem.unitPrice
    };
      ...
      items: prev.items.map(item => {
        if (item.id === itemId) {
          ...
          if (field === 'quantity' || field === 'unitPrice') {
            updated.total = updated.type === "part" ? updated.quantity * updated.unitPrice * (1 + (settings.pricing?.partsMarkup || 0.35)) : updated.quantity * updated.unitPrice;
          }
          return updated;
        }
        return item;
