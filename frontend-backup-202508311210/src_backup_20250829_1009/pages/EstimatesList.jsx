...
export default function EstimatesList() {
  ...
  const loadEstimates = async () => {
    try {
      setLoading(true);
      const response = await apiEndpoints.estimates.getAll();
  ...
  const exportToCSV = () => {
    try {
      const itemsToExport = filteredItems || [];
const csvData = itemsToExport.map(est => ({      customerName: est.customerName || '',      vehicleInfo: est.vehicleInfo || '',      status: est.status || 'draft',      total: parseFloat(est.total || 0).toFixed(2),      date: est.date ? format(new Date(est.date), 'MM/dd/yyyy') : ''    }));
