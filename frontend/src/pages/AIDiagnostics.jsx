...
import aiDiagnosticsService from "../utils/aiDiagnosticsService";
...
const AIDiagnostics = () => {
  ...
  const handleQuickDiagnosis = async () => {
    ...
    try {
      // Use the aiDiagnosticsService correctly
      const response = await aiDiagnosticsService.generateQuickDiagnosis({
        vehicle: vehicleInfo,
      ...
      if (response.success) {
        ...
        setError(response.message || 'Diagnosis failed. Please try again.');
      }
    } catch (err) {
  ...
  const handleObdLookup = async () => {
    ...
    if (!aiDiagnosticsService.validateOBDCode(obdCode)) {
      setError('Invalid OBD code format. Use format like P0171, P0420, B1000, etc.');
      return;
    }
    ...
    try {
      ...
      const response = await aiDiagnosticsService.lookupOBDCodes(obdCode.toUpperCase());

      if (response.success || response.code) {
        setObdResults(response);
      } else {
        setError('OBD code lookup failed. Please try again.');
      }
    } catch (err) {
      console.error('OBD lookup error:', err);
      
      // Fallback to common OBD codes
      const commonCodes = aiDiagnosticsService.getCommonOBDCodes();
      const allCodes = { ...commonCodes.powertrain, ...commonCodes.body, ...commonCodes.chassis, ...commonCodes.network };
      
      if (allCodes[obdCode.toUpperCase()]) {
        setObdResults({
          code: obdCode.toUpperCase(),
          description: allCodes[obdCode.toUpperCase()],
          severity: aiDiagnosticsService.getOBDCodeSeverity(obdCode.toUpperCase()),
          fallback: true
        });
      } else {
        setError('OBD code not found in database. Please verify the code and try again.');
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-gray-700 mt-1 capitalize">{obdResults.severity || 'Unknown'}</p>
            </div>
            {obdResults.causes && (
              <div>
            ...
            {obdResults.fallback && (
              <p className="text-sm text-gray-500 italic">Using local database</p>
            )}
          </div>
        </div>
      )}
