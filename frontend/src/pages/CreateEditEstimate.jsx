...
import DocumentTemplplateRenderer from "../components/DocumentTemplplateRenderer";
...
export default function CreateEditEstimate() {
  ...
  const addPart = () => setEstimate((e) => ({ ...e, parts: [...e.parts, { name: "", partNumber: "", quantity: 1, cost: "" }] }));
  const addLabor = () => setEstimate((e) => ({ ...e, labor: [...e.labor, { description: "", hours: "", rate: settings.laborRate }] }));
                ...
                {estimate.parts.map((p, i) => (
                  <tr key={i} className="border-b">
                    ...
                    <td className="p-2 text-right">
                      <input type="number" className="w-24 border rounded p-1 text-right" value={p.quantity ?? 1}
                             onChange={(e) => {
                               const parts = [...estimate.parts];
