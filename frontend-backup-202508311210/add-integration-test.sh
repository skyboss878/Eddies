...
# Add integration test component to your app
echo "🔧 Adding Integration Test component to your app..."

cd ~/eddies-asian-automotive/frontend

# Create the integration test component
...
# Add the component to the components index
echo ""
echo "🔧 Adding IntegrationTest to components index..."

# Add export to components/index.js
if ! grep -q "IntegrationTest" src/components/index.js; then
    echo "export { default as IntegrationTest } from './testing/IntegrationTest';" >> src/components/index.js
    echo "✅ Added IntegrationTest export"
else
    echo "✅ IntegrationTest already exported"
fi

# Create a simple route addition guide
echo ""
echo "🛣️ To add this test to your app, add this route to your App.jsx:"
echo ""
echo "Import:"
echo "import { IntegrationTest } from './components';"
echo ""
echo "Route (add inside your <Routes>):"
echo '<Route path="/integration-test" element={<IntegrationTest />} />'
echo ""

# Create a quick add route script
read -p "Would you like me to automatically add the integration test route to your App.jsx? (y/n): " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "🔧 Adding route to App.jsx..."
    ...
    if [ -f "src/App.jsx" ]; then
        ...
        if ! grep -q "IntegrationTest" src/App.jsx; then
            # Find the line with other component imports and add after it
            if grep -q "from.*components" src/App.jsx; then
                sed -i "/from.*components/a import { IntegrationTest } from './components';" src/App.jsx
            else
        ...
        if ! grep -q "/integration-test" src/App.jsx; then
            # Find where other routes are and add before the closing </Routes>
            if grep -q "</Routes>" src/App.jsx; then
                ...
                echo "⚠️ Could not find </Routes> - please add the route manually"
            fi
        else
            echo "✅ Integration test route already exists"
        fi
        
        echo "✅ App.jsx updated successfully"
    else
        echo "❌ App.jsx not found - please add the route manually"
    fi
fi

echo ""
echo "🎉 Integration test component created!"
echo ""
echo "📋 What was created:"
echo "  ✅ src/components/testing/IntegrationTest.jsx"
...
echo ""
echo "🚀 How to use:"
echo "1. Start your backend: cd ~/eddies-asian-automotive/backend && python app.py"
echo "2. Start your frontend: npm start"
echo "3. Visit: http://localhost:3000/integration-test"
echo "4. Click 'Run Integration Tests' to test the connection"
echo ""
echo "🔍 The test will check:"
...
echo ""
echo "💡 Check browser console and network tab for detailed error information"
