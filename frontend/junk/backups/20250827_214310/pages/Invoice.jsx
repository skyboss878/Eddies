...
function Invoice() {
  ...
  useEffect(() => {
    async function fetchInvoice() {
      ...
      setLoading(true);
      try {
        const job = await apiEndpoints.jobs.getById(jobId);
        const [vehicle, customer] = await Promise.all([
          apiEndpoints.vehicles.getById(job.vehicleId),
          apiEndpoints.customers.getById(job.customerId),
        ]);
  ...
  const handleEmailInvoice = () => {
    showCustomConfirm("Email this invoice to the customer?", async () => {
      try {
        await apiEndpoints.jobs.emailInvoice?.(jobId) // TODO: Add emailInvoice method;
    ...
    <div className="p-6 max-w-4xl mx-auto bg-white text-gray-900 rounded-lg shadow-lg my-8 overflow-auto print:shadow-none print:my-0" ref={printRef}>
        ...
        <div className="mb-6">
          ...
          <table className="w-full text-sm border border-gray-300">
            <thead className="bg-gray-100">
              <tr key={i}><th>Item</th><th>Qty</th><th>Unit</th><th>Total</th></tr>
            </thead>
            <tbody>
              {invoiceData.parts.map((p, i) => (
                <tr key={i} key={i} className="text-right">
      ...
      {invoiceData.labor.length > 0 && (
        <div className="mb-6">
          ...
          <table className="w-full text-sm border border-gray-300">
            <thead className="bg-gray-100">
              <tr key={i}><th>Description</th><th>Hours</th><th>Rate</th><th>Total</th></tr>
            </thead>
            <tbody>
