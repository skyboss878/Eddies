// src/contexts/DataContext.jsx - FIXED VERSION WITH CORRECT API CALLS
...
export const DataProvider = ({ children }) => {
  ...
  const safeApiCall = useCallback(async (apiCall, defaultValue = []) => {
    try {
  ...
  const fetchAllData = useCallback(async (force = false) => {
    ...
    try {
      console.log('🔄 Fetching all shop data...');

      // Fetch data with safe wrappers
      const [
        customers,
        vehicles,
        jobs,
        ...
        safeApiCall(() => customerService.getAll(), []),
        safeApiCall(() => vehicleService.getAll(), []),
        safeApiCall(() => jobService.getAll(), []),
        safeApiCall(() => estimateService.getAll(), []),
        safeApiCall(() => partService.getAll(), []),
        safeApiCall(() => laborService.getAll(), []),
        safeApiCall(() => settingsService.getAll(), {}) // Fixed: use getAll() instead of get()
      ]);
      ...
      setState(prev => ({
        ...
        laborRates: Array.isArray(laborRates) ? laborRates : [],
  ...
  }, [isAuthenticated, state.loading.initialData, state.dataLoaded, setLoading, setError, safeApiCall]);
    ...
    return {
      create: async (data) => {
        ...
        try {
          ...
          console.error(`Failed to create ${entityKeySingular}:`, error);
      ...
      update: async (id, data) => {
        ...
        try {
          ...
          console.error(`Failed to update ${entityKeySingular}:`, error);
      ...
      delete: async (id) => {
        ...
        try {
          ...
          console.error(`Failed to delete ${entityKeySingular}:`, error);
  ...
  const updateSettings = useCallback(async (settingsData) => {
    ...
    try {
      const result = await settingsService.updateMultiple(settingsData); // Fixed: use correct method
