...
export default function UserDropdown() {
  ...
  if (user.role === 'admin') {
    roleLinks.push(
      ...
      { label: 'Diagnostics', to: '/diagnostics', icon: WrenchScrewdriverIcon }
    );
  } else if (user.role === 'technician') {
    roleLinks.push(
      { label: 'Time Clock', to: '/timeclock', icon: WrenchScrewdriverIcon }
    );
  }
  ...
  return (
    <Menu as="div" className="relative">
      <Menu.Button className="flex items-center space-x-1 sm:space-x-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white focus:outline-none">
        ...
        <span className="hidden sm:inline truncate max-w-20 lg:max-w-none">{displayName}</span>
            ...
            {roleLinks.map(link => (
              <Menu.Item key={link.to}>
                {({ active }) => (
                  ...
                  >
                    <link.icon className="mr-3 h-5 w-5 text-gray-400 flex-shrink-0" />
                    {link.label}
                  </Link>
                )}
              </Menu.Item>
