...
import { useDataOperations, useLocalStorage } from "../hooks";
import { Wrench, FileText, Send, Trash2 } from 'lucide-react';
...
export default function Diagnosis() {
  ...
  const handleDiagnosis = async () => {
    ...
    if (result.success) {
      const newResult = {
        ...
        ...result.data, // diagnosis, fix, parts, labor, confidence
  ...
  const createJobFromDiagnosis = () => {
    if (!latestDiagnosis || !selectedVehicleId)
      return utils.showMessage('Please complete a diagnosis and select a vehicle first', 'error');
            ...
            {latestDiagnosis ? (
              <div className="space-y-4">
                ...
                <div className="flex gap-3 pt-3">
                  <button
                    onClick={createJobFromDiagnosis}
                    className="flex-1 px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2"
                  >
        ...
        {diagnosisHistory.length > 0 && (
          <div className="mt-8 bg-black/60 rounded-xl shadow-lg border border-gray-700 p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => setDiagnosisHistory([])}
                className="flex items-center text-sm px-3 py-1 bg-red-700 rounded hover:bg-red-800"
              >
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto pr-2">
              {diagnosisHistory.map((d) => (
                <button
                  key={d.id}
                  onClick={() => setLatestDiagnosis(d)}
                  className="w-full text-left p-3 bg-gray-800 rounded-lg border border-gray-600 hover:bg-gray-700 transition-colors flex justify-between items-start"
                >
                  <div>
                    <p className="text-gray-200 font-medium truncate">{d.description}</p>
