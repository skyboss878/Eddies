...
const Dashboard = () => {
  ...
  useEffect(() => {
    const calculateStats = () => {
      ...
      const activeJobsCount = Array.isArray(jobs) 
        ? jobs.filter(j => j.status === 'in_progress' || j.status === 'active').length 
        : 0;
    ...
    if (!loading.customers && !loading.vehicles && !loading.jobs && !loading.estimates && !loading.invoices) {
      calculateStats();
    }
  ...
  const smartAlerts = useMemo(() => {
    if (loading.customers || loading.jobs || loading.invoices || loading.estimates) {
    ...
    const overdueInvoices = Array.isArray(invoices) ? 
      invoices
        .filter(inv => inv.status !== 'paid' && (now - new Date(inv.created_at)) / oneDay > 30)
        .slice(0, 5) : [];

    const pendingEstimates = Array.isArray(estimates) ? 
      estimates.filter(est => est.status === 'pending').slice(0, 5) : [];

    const upcomingJobs = Array.isArray(jobs) ? 
      jobs.filter(job => {
        if (!job.scheduled_at && !job.created_at) return false;
  ...
  const recentItems = useMemo(() => {
    const sortByDate = (arr) => Array.isArray(arr) 
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="relative w-full md:w-auto md:min-w-[350px]">
          <div className="flex items-center">
            <input
              type="text"
              className="w-full border border-gray-300 rounded-md p-2 pr-8 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          ...
          {activeTab === 'invoices' && (
            recentItems.invoices.length > 0 ? recentItems.invoices.map(inv => (
              <div key={inv.id} className="p-3 border-b last:border-b-0 flex justify-between items-center">
                <span>Invoice #{inv.id} - <span className="font-mono text-sm bg-gray-100 p-1 rounded">{inv.status || 'unknown'}</span></span>
          ...
          {activeTab === 'estimates' && (
            recentItems.estimates.length > 0 ? recentItems.estimates.map(est => (
              <div key={est.id} className="p-3 border-b last:border-b-0 flex justify-between items-center">
                <span>Estimate #{est.id} - <span className="font-mono text-sm bg-gray-100 p-1 rounded">{est.status || 'unknown'}</span></span>
